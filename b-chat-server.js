...
app.get("/api/users/all", auth, (req, res) => {
  const users = db.prepare("SELECT id,username,name,bio,avatar,online,last_seen FROM users WHERE id!=? ORDER BY online DESC, name ASC LIMIT 100").all(req.user.id);
  res.json(users);
});
...
app.post("/api/messages/send", auth, (req, res) => {
  ...
  if (!content || !conv_id) return res.status(400).json({ error: "Missing fields" });

  const member = db.prepare("SELECT id FROM conv_members WHERE conv_id=? AND user_id=?").get(conv_id, req.user.id);
  ...
  const msgId = uuidv4();
  const now = Math.floor(Date.now() / 1000);
  const seenBy = JSON.stringify([req.user.id]);

  db.prepare(`INSERT INTO messages (id,conv_id,sender_id,sender_name,type,content,reply_to_id,reply_to_name,reply_to_content,seen_by,created_at)
    VALUES (?,?,?,?,?,?,?,?,?,?,?)`)
    .run(msgId, conv_id, req.user.id, sender.name, "text", content,
      reply_to_id || null, reply_to_name || null, reply_to_content || null, seenBy, now);
  ...
  // Send to ALL OTHER members via WebSocket
  const members = db.prepare("SELECT user_id FROM conv_members WHERE conv_id=? AND user_id!=?").all(conv_id, req.user.id);
  members.forEach(m => sendToUser(m.user_id, msgData));

  res.json({ id: msgId, conv_id, sender_id: req.user.id, sender_name: sender.name, content, created_at: now, seen_by: [req.user.id] });
});
